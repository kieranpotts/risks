---
name: land-session
description: Land a threat modeling session — merge its pull request into main once review is settled. Use when the user says "land this session", "merge the session", "the threat model is ready", or "land the session PR". Do not use this skill to scaffold a session — use draft-session for that. Do not use it to update a risk — use update-register.
license: MIT
metadata:
  interactive: yes
---
...
Do NOT use this skill to scaffold a session — use [`/draft-session`](../draft-session/SKILL.md).
...
-   **Squash-merge with the conventional message.**
    ...
    `session: <short lowercase description>`. The report, its `INDEX.md` row, and the register rows were all added together in [`/draft-session`](../draft-session/SKILL.md), so no further index or register update is needed at merge time.
...
- `risks/INDEX.md` on `main` includes the new session's row, and `risks/REGISTER.md` includes its new rows (both already present from `/draft-session`, now landed).

## References

- [`AGENTS.md`](../../../AGENTS.md): The full session workflow.

- [`draft-session`](../draft-session/): Scaffolds the session this skill lands.
